...
def preprocessing_1(df, type=1):
    ...
    df.replace(to_replace=0, value=0.01, inplace=True)
...
def pointwise_preprocessing(df, columns, test=False):
    ...
    idcs += [columns['site_id'], columns['visitor_location_country_id'], columns['prop_id'], columns['prop_country_id'],
             columns['srch_destination_id']]
