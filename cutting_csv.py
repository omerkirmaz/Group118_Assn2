...
class CuttingData:
    ...
    def cutting(self):
        ...
        for i in range(round(int(length_total / 20))):
            ...
            if i % 100 == 0:
                print(f"""{i} of {round((length_total / 20))} search ids appended to the dataframe
-----------------------------------------------------------------------------------------------------------------------
""")

    def saving_csv(self):
        self.shortened_df.to_csv(r'data/shortened_data/data_10000.csv', index=False, header=True)
