...
class PreProcess:
    ...
    def price_usd(self):
        ...
        visitor_location_df = self.df[self.df["prop_country_id"].notna()]
        location_5 = visitor_location_df[visitor_location_df['prop_country_id'] == 219]

        #  print(visitor_location_df['visitor_location_country_id'].unique())
        #  print(visitor_location_df['prop_country_id'].value_counts().to_string())
        ...
        for row in range(len(per_night_df['price_usd'])):
            ...
            percent = (total_booking + price*stay) // 2
